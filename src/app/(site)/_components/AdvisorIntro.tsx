...
export function AdvisorIntro({ agent }: { agent: Agent }) {
  return <section id="sobre" className="shell section-space" aria-labelledby="advisor-heading"><div className="advisor-layout">
    ...
    <Reveal><span className="eyebrow mb-5 text-olive">O olhar por trás da seleção</span><h2 id="advisor-heading" className="display section-title">Imóveis são sobre<br /><em>pessoas.</em></h2><p className="mt-7 max-w-md text-base leading-7">{agent.headline}</p><p className="mt-5 max-w-md text-[13px] leading-7 text-muted">{agent.bio}</p><p className="advisor-signature mt-7">{agent.name}</p>
